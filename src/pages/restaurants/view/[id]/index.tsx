...
import { deleteManagerById, createManager } from 'apiSdk/managers';
...
function RestaurantViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RestaurantInterface>(
    ...
    () =>
      getRestaurantById(id, {
        relations: ['user', 'reservation', 'manager'],
      }),
  );
  ...
  const managerHandleCreate = async () => {
    setCreateError(null);
    try {
      await createManager({ restaurant_id: id, user_id: managerUserId });
  ...
  const managerHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              <Stack spacing={2} mb={8}>
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect name={'manager_user'} value={managerUserId} handleChange={setManagerUserId} />
                  </Box>
                  <Button colorScheme="blue" mt="4" mr="4" onClick={managerHandleCreate} isDisabled={!managerUserId}>
                    Create
                  </Button>
                </Flex>
                <TableContainer>
